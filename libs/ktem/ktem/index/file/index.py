    ...
    def delete_source(self, source_id: str):
        with Session(engine) as session:
            source = session.execute(
                self._resources["Source"].__table__.select().where(
                    self._resources["Source"].id == source_id
                )
            ).first()
            if source:
                session.execute(
                    self._resources["Source"].__table__.delete().where(
                        self._resources["Source"].id == source_id
                    )
                )
            vs_ids, ds_ids = [], []
            index_rows = session.execute(
                self._resources["Index"].__table__.select().where(
                    self._resources["Index"].source_id == source_id
                )
            ).all()
            for each in index_rows:
                relation_type = each.relation_type
                if relation_type == "vector":
                    vs_ids.append(each.target_id)
                elif relation_type == "document":
                    ds_ids.append(each.target_id)

            session.execute(
                self._resources["Index"].__table__.delete().where(
                    self._resources["Index"].source_id == source_id
                )
            )
            session.commit()

        if vs_ids and self._vs:
            self._vs.delete(vs_ids)
        if ds_ids:
            self._docstore.delete(ds_ids)
