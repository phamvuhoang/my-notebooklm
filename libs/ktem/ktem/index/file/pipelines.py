...
class IndexPipeline(BaseComponent):
    ...
    def finish(
        ...
        with Session(engine) as session:
            ...
            if source_note:
                item.note.update(deepcopy(source_note))
    ...
    def stream(
        ...
        **kwargs,
        ...
        file_id = existing_source_id or self.get_id_if_exists(
        ...
        if isinstance(file_path, Path):
            if file_id is not None:
                if not reindex:
                    raise ValueError(
                        f"File {display_name} already indexed. Please rerun with "
                        "reindex=True to force reindexing."
                    )
                else:
                    # remove the existing records
                    yield Document(
                        f" => Removing old {display_name}", channel="debug"
                    )
            ...
            if file_id is not None:
                raise ValueError(f"URL {display_name} already indexed.")
            else:
                # add record to db
                file_id = self.store_url(
                    file_path,
                    source_name=source_name,
                    source_note=source_note,
                )
        ...
        if source_name:
            extra_info["file_name"] = source_name
        if metadata_overrides:
            extra_info.update(metadata_overrides)
        ...
        self.finish(file_id, file_path, source_note=source_note)
