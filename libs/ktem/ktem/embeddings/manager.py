...
def _merge_seed_spec(existing_spec: dict, seeded_spec: dict) -> tuple[dict, bool]:
    merged = dict(existing_spec or {})
    changed = False

    for key, value in (seeded_spec or {}).items():
        current = merged.get(key)
        if key == "model" and current in _DEPRECATED_EMBEDDING_MODELS:
            replacement = _DEPRECATED_EMBEDDING_MODELS[current]
            ...
            for name, model in flowsettings.KH_EMBEDDINGS.items():
                with Session(engine) as sess:
                    ...
                    if item is None:
                        ...
                        merged_spec, changed = _merge_seed_spec(item.spec, model["spec"])
                        if changed:
                            item.spec = merged_spec
                            sess.add(item)
                            sess.commit()
